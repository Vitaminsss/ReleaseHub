...
- 首次生成 `.env`（含 `JWT_SECRET`、`ADMIN_PASSWORD_HASH`、`RELEASES_DIR`（指向本目录下 `releases/`）、`BASE_URL`、`PORT`）。启用 Nginx 且无路径前缀时首次 `BASE_URL` 多为 `http://<公网IP>`（无端口）；有前缀时为 `http://<公网IP>/<前缀>`；未启用 Nginx 时为 `http://<公网IP>:3721`。
- PM2 进程名：`release-hub`；防火墙在启用 Nginx 时通常放行 **80** 与 **3721**。
...
配置 HTTPS 后，在后台将 **BASE_URL** 改为 `https://你的域名`。
